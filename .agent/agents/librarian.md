...
To preserve strict domain boundaries, the Technical Writer possesses write permissions for documentation but is restricted from editing application source files in `src/`. Any transaction targeting execution paths inside `src/` must follow standard safety policies.
