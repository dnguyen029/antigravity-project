...
- **Archive Verification**: Automatically check Supermemory status before deletion to ensure no "Zero-Amnesia" occurs.
...
```bash
node scripts/lifecycle/cleanup.js --dry-run  # Check before deleting
node scripts/lifecycle/cleanup.js --force    # Perform actual deletion
```
...
_Certified by @orchestrator - Version 1.0.0_
